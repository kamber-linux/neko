...
# END OF FILE
# len_uncompressed should be the uncompressed length. However, in at least
# version 4.3 of RFC1952, it is instead defined as the length of the
# uncompressed file modulo 2^32 (4294967296), so that files above 4GB in
# uncompressed size will successfully compress and decompress. The reference
# implementation of gzip had a patch for older versions that removed the length
# error to handle files above 4GB, with the modulo part of RFC1952 probably
# coming later to clarify this behavior. The original name of the variable in
# the source code is ISIZE. That is: input size - which doesn't aid in clearing
# things up. Some implementations still do have a 4GB size limit.
#
# On another note, it's hard to find older versions or RFC1952, so old code
# might be the only available reference. This is a later topic of research.
#
